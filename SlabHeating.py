...
def heating(xH2O = 0, np = 0, T1bar = 0, return_diag = 0):
    ...
    data = SaturnData()
    data.import_rss(pmin = 10.E3, pmax = 1.E5)
...
class SlabHeating(ShallowWater):
    id = '2'
    ...
    def set_variables(self):
        ...
        x0 = self.xlen/2. * (1 - 0.4)
        absorb = zeros((self.nx, self.ny))
        for i, x in enumerate(self.xaxis):
            if abs(x) <= x0:
                absorb[i, :] = 0
            else:
                absorb[i, :] = self.gamma * (exp(((abs(x) - x0) / (self.xlen/2. - x0))**4) - 1)
...
if __name__ == '__main__':
    ...
    ax = draw_prepare()
    T2 = fsolve(lambda x: x - compute(ax, x, xH2O) - 134.8, 150)
    clf()
    #T2 = 144.86

    diag = heating(xH2O = xH2O, np = ny, T1bar = T2, return_diag = 1)
    xaxis = linspace(-xlen/2., xlen/2., nx)
    yaxis = diag.zlev
    ylen  = yaxis[-1] - yaxis[0]
    model = SlabHeating(name, nx, ny, xlen, ylen, tstart, tend, tstep, frame)
